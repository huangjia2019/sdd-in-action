...
### New Capabilities
- `analyzer-retry-policy`: 在 LLM API 调用层提供指数退避重试和降级处理，确保瞬时故障不影响整体 pipeline 执行
...
**受影响文件**:
- `pipeline/model_client.py`: 新增 `with_retry` 装饰器和相关异常处理
- `pipeline/pipeline.py`: `step_analyze` 函数需要处理 `degraded` 状态项
- `tests/test_retry.py`: 新增测试验证重试逻辑和降级行为

**依赖项**: 无新增外部依赖（仅使用标准库 `time` 和 `random` 实现退避和抖动）

**API 变更**: 无公共 API 变更，`chat()` 函数签名保持不变，仅内部行为增强