...
这里是黄佳(咖哥)新书 **《SDD 实战:规范驱动开发之道》** 的官方配套代码仓库,以及对应的 SDD 行动营四周公开实操材料。
...
AI 编程进入深水区。**第一天效率飙升,第一周开始返工,第一个月陷入自我怀疑。** 当 AI 一次性吐出几百行代码,而你说不清当初为什么这么写、又如何复现昨天的产出时,问题已经不是 AI 能力不够,而是开发方法论缺了一环——**「规范」这一环**。

这本书把答案系统化了:**SDD(Specification-Driven Development,规范驱动开发)** 把 Stephen Covey "以终为始、两次创造"的心法翻译为 `proposal.md` / `design.md` / `tasks.md` 三份可执行的工程文档,让 AI 时代的规范不再是文档归档处吃灰的 Word 文件,而是 Agent 真正读取、解析、执行的**机器接口**。
